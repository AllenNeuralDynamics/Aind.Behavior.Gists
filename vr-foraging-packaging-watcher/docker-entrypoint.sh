...
# shipped in this image and the files available under $HOME/.aws (the read-only
# bind-mount from the host, whatever the host-side directory happens to be called).
# $HOME (not /root) so this also works when the container runs as a non-root UID
# (needed for NFS exports whose ACLs don't grant root access -- see docker-compose.yml).
if [ -f "$HOME/.aws/config" ]; then
    sed \
        -e 's|[A-Za-z]:[/\\][Uu]sers[/\\][^/\\]*[/\\]\.aws[/\\]aws_signing_helper\.exe|/usr/local/bin/aws_signing_helper|g' \
        -e "s|[A-Za-z]:[/\\\\][Uu]sers[/\\\\][^/\\\\]*[/\\\\]\.aws[/\\\\]\?|$HOME/.aws/|g" \
        -e 's|/[^[:space:]]*/\.aws/aws_signing_helper|/usr/local/bin/aws_signing_helper|g' \
        -e "s|/[^[:space:]]*/\.aws/|$HOME/.aws/|g" \
        "$HOME/.aws/config" > /tmp/aws-config-linux
    export AWS_CONFIG_FILE=/tmp/aws-config-linux
    echo "[entrypoint] \$HOME/.aws/config patched for Linux (Windows paths → \$HOME/.aws/, signing helper → Linux binary)"
fi
