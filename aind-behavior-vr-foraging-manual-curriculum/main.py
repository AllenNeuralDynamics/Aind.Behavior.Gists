"""Batch-override VR Foraging curriculum suggestions in Dataverse.

For each subject, build the custom TrainerState (see curriculum.py) and append it as the newest
suggestion, overriding whatever the rig last pushed.

Usage:
    uv run main.py --subjects 828426 841306 828415
    uv run main.py --subjects-file subjects.txt
    uv run main.py --subjects 828426 --dry-run      # build + validate, push nothing
    uv run main.py --subjects 828426 --yes          # skip the confirmation prompt
...
from curriculum import build_trainer_state
...
def main() -> int:
    ...
    console.print(f"Building trainer states for [bold]{len(subjects)}[/bold] subject(s)...")
    ...
    table = Table(title="Override plan")
